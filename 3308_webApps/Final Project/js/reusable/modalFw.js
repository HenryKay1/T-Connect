...
// It will adorn the public object with a couple of public methods.
    ...
    modalFw.setBackgroundColor = function(color){
        modalWindow.style.backgroundColor = color;
    };

    // create public method that can be used by the HTML coder.
    modalFw.alert = function (heading,message) {
        show(modalWindow);
        console.log("function modalFw.alert was called with heading " + heading + "for message"+ message );
        ...
        var headingArea = document.createElement("b");  //Used b instead of h(n) for proportions
        headingArea.innerHTML = heading;
        var messageArea = document.createElement("p"); // add message area
        messageArea.innerHTML = message;
        
       modalWindow.appendChild(headingArea);
    ...
    modalFw.confirm = function (message, okFunction) {
        ...
        okButton.setAttribute("value", "Yes");
        ...
        cancelButton.setAttribute("value", "No");
