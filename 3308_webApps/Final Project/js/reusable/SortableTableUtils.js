...
SortableTableUtils.makeNumText = function (numText) { //For numbers that aren't currency
    var tableData = document.createElement('td');
    numText = numText || ""; // set it to empty string if the input parameter does not exist
    if (numText === "") {
        tableData.sortOrder = -1;  // put empty entries at top if sorting by this column
    } else {
        tableData.sortOrder = numText.toUpperCase();
    }
    tableData.innerHTML = numText;
    tableData.style.textAlign = "right"; // text elements usually align left
    tableData.filterValue = numText;
    return tableData;
};
